...
class DDPG:
    ...
    def sample_action(self,mean,std):
        ...
        discrete_steps = self.env.action_space.n
        buckets = torch.linspace(-1,1,discrete_steps+1,device=device)
        action = torch.tensor([torch.where(buckets <= a)[0][-1] for a in action],device=device)
    ...
    def train(self):
        ...
        for i in range(episode+1, 1000):
            ...
            for t in count():
                ...
                if terminated or truncated or trajectory_length > 1000:
                    break
    ...
    def save_checkpoint(self,episode,beta,rewards):
        ...
        torch.save(checkpoint, os.path.join("Acrobat", f"taylor_{episode}.pth"))
...
plt.plot(rewards)
plt.savefig("taylor_mountain_1k.png")
